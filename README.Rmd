...
- *nx*: any number of *chr* or *num* columns numbered sequentially (i.e. *n1*, *n2*, *n3*, ...). Could contain the number of observations and/or summary per arm.
...
The *nx* columns include the number of observations and descriptives (mean (sd)) for each arm, *beta* is a mean difference, *p1* the p-value.
...
Boxplot layout can be controlled via *bpopt* (currenlty just colour and width),
	the additional x-axis via *bplim*, *bplab* and *bplab_text*;
...
The points can be controlled via *prps*, the additional x-axis via *plim*, *plab* and *plab_text*; 
	lines at the side of the plot can be added via *sideline*.
